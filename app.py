        ...
        for key in keys:
            ...
            if data:
                try:
                    ...
                    data["originalTotal"] = int(data.get("originalTotal", data.get("total", 0)))
                    data["discount"] = int(data.get("discount", 0))
                    try:
    ...
    try:
        ...
        if data:
            ...
            if "referredBy" in data:
                try:
                    data["referredBy"] = int(data["referredBy"]) if data["referredBy"] else None
                except:
                    data["referredBy"] = None
        return data
    except Exception as e:
...
def save_user(user_data):
    try:
        ...
        # Return original referrals format
...
def add_bonus_to_user(user_id, bonus_amount):
    try:
        key = f"{USERS_KEY}:{user_id}"
        if redis_client.exists(key):
            current_bonus = int(redis_client.hget(key, "bonus") or 0)
            new_bonus = current_bonus + bonus_amount
            redis_client.hset(key, "bonus", new_bonus)
            redis_client.hset(key, "updated_at", get_current_time())
            return True
        return False
    except Exception as e:
        print(f"Error add_bonus_to_user: {e}")
        return False

def add_referral(referrer_id, referred_id):
    try:
        referrer_key = f"{USERS_KEY}:{referrer_id}"
        if redis_client.exists(referrer_key):
            referrals_json = redis_client.hget(referrer_key, "referrals") or "[]"
            try:
                referrals = json.loads(referrals_json)
            except:
                referrals = []
            
            if referred_id not in referrals:
                referrals.append(referred_id)
                redis_client.hset(referrer_key, "referrals", json.dumps(referrals))
                redis_client.hset(referrer_key, "updated_at", get_current_time())
                return True
        return False
    except Exception as e:
...
def get_stats():
    try:
        ...
        if stats:
            for k in ["total_orders", "total_products", "total_users", "total_revenue", "pending_orders"]:
...
# ================== REFERRAL SYSTEM ==================
def process_referral_bonus(user_id, order_total):
    try:
        ...
        bonus_amount = int(order_total * 0.1)  # 10% bonus
        
        # Add bonus to referrer
        add_bonus_to_user(referrer_id, bonus_amount)
        
        # Check if this is user's first order
        user_orders = get_orders_by_user(user_id)
        if len(user_orders) == 1:  # First order
            # Add bonus to new user
            add_bonus_to_user(user_id, bonus_amount)
...
def api_create_order():
    ...
    if order:
        ...
        process_referral_bonus(order["userId"], order["total"])
...
def api_update_order_status(oid):
    data = request.json
    success = update_order_status(oid, data.get("status", "pending"))
    if success:
...
def api_get_user(uid):
    ...
    if ref_code and ref_code.startswith("REF"):
        try:
            ...
            if referrer_id != uid and get_user(referrer_id):  # Valid referrer
...
def api_update_user(uid):
    ...
    if "bonus_add" in data:
        bonus_add = data.pop("bonus_add")
        add_bonus_to_user(uid, bonus_add)
    
    # Handle referral order processing
    if "referral_order" in data:
        referral_user_id = data.pop("referral_order")
        # This is handled in process_referral_bonus
...
# REFERRAL ROUTES
@app.route("/api/referral/<ref_code>", methods=["GET"])
def api_get_referral_info(ref_code):
    ...
    try:
        ...
        if referrer:
            return jsonify({
                "valid": True,
                "referrer": referrer.get("username", f"User {referrer_id}"),
                "bonus": "10%"
            })
    except:
        pass
...
def init_sample_data():
    ...
    if not get_all_promos():
        sample_promos = [
            {"code": "WELCOME10", "discount": 10, "uses": 100},
            {"code": "NEWUSER15", "discount": 15, "uses": 50},
            {"code": "SAVE20", "discount": 20, "uses": 25}
        ]
        for promo in sample_promos:
            save_promo(promo)
        print("✅ Sample promos added")
